...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const heroUrl = params.get('hero_url');
    const genre = params.get('genre');
    const webhook = params.get('webhook');

    if (webhook) webhookRef.current = webhook;
    if (genre && GENRES.includes(genre)) setSelectedGenre(genre);
  ...
  useEffect(() => {
      if (shouldAutoLaunch && hero) {
          launchStory();
          setShouldAutoLaunch(false);
      }
  }, [shouldAutoLaunch, hero]);
  ...
  const postToWebhook = async (imageUrl: string, pageIndex: number) => {
      if (!webhookRef.current) return;
      try {
          const blob = await (await fetch(imageUrl)).blob();
          const formData = new FormData();
          formData.append('file', blob, `panel-${pageIndex}.jpg`);
          formData.append('payload_json', JSON.stringify({ 
              content: `**Page ${pageIndex}** generated!`, 
              username: "Infinite Heroes Bot" 
          }));
          await fetch(webhookRef.current, { method: 'POST', body: formData });
      } catch (e) {
          console.error("Webhook post failed", e);
      }
  };

  const postPDFToWebhook = async () => {
      if (!webhookRef.current) return;
      try {
        const PAGE_WIDTH = 480;
        const PAGE_HEIGHT = 720;
        const doc = new jsPDF({ orientation: 'portrait', unit: 'pt', format: [PAGE_WIDTH, PAGE_HEIGHT] });
        const pagesToPrint = historyRef.current
        ...
        const formData = new FormData();
        formData.append('file', pdfBlob, 'Infinite-Heroes-Issue.pdf');
        formData.append('payload_json', JSON.stringify({ 
            content: `**Full Issue PDF** is ready!`, 
            username: "Infinite Heroes Bot" 
        }));
        await fetch(webhookRef.current, { method: 'POST', body: formData });
      } catch (e) {
          console.error("PDF Webhook post failed", e);
      }
  };
...
`;
  ...
  const generateSinglePage = async (faceId: string, pageNum: number, type: ComicFace['type']) => {
      ...
      if (url && webhookRef.current) {
          postToWebhook(url, pageNum);
          if (type === 'back_cover') postPDFToWebhook();
      }
  };
